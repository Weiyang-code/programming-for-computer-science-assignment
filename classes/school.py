...
class School:
    ...
    def menu(self):
        ...
        if os.path.exists(f"data/{student_path}"):
            ...
            pd.DataFrame(columns=['student_id', 'name', 'email', 'contact', 'class_name', 'age']).to_csv(f"data/{student_path}", index=False)
        ...
        if user == 'admin':
            ...
            admin_data = pd.read_csv('data/admin.csv', dtype={'id': str, 'password': str})

            if ((admin_data['id'] == id) & (admin_data['password'] == password)).any():
                print("Login successful!")
                admin1 = Admin(id=id, password=password)
            ...
            teacher_data = pd.read_csv('data/teacher_credentials.csv', dtype={'id': str, 'password': str})

            if ((teacher_data['id'] == id) & (teacher_data['password'] == password)).any():
                print("Login successful!")
                teacher1 = Teacher(id=id, password=password)
